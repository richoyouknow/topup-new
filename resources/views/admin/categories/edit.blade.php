...
<div class="flex flex-col gap-8 max-w-3xl">
    <div>
        <h1 class="text-2xl font-extrabold text-white">Edit Kategori</h1>
        <p class="text-xs text-gray-500 mt-1">Perbarui nama, deskripsi, atau gambar kategori.</p>
    </div>
    ...
    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="bg-card-dark border border-border-dark rounded-2xl p-6 sm:p-8 shadow-glow flex flex-col gap-6">
        @csrf
        @method('PUT')
        ...
        <div class="flex flex-col gap-2">
            ...
            <div class="p-4 bg-slate-950 border border-border-dark rounded-xl flex items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    <img src="{{ $category->image_path }}" alt="{{ $category->name }}" class="w-12 h-12 object-contain rounded-lg border border-border-dark bg-slate-900 p-1">
            ...
            <div class="relative flex flex-col items-center justify-center p-5 border border-dashed border-border-dark hover:border-primary-purple/40 rounded-xl bg-slate-950/40 cursor-pointer transition-all group">
                ...
                <!-- Upload Prompt -->
                <div class="flex flex-col items-center text-center gap-1 {{ $category->image_path ? 'hidden' : '' }}" id="upload-prompt">
                ...
                <!-- Image Preview -->
                <div class="flex flex-col items-center gap-1.5 {{ $category->image_path ? 'hidden' : '' }}" id="upload-preview-container">
                    <img src="{{ $category->image_path ?? '' }}" id="upload-preview-img" class="w-20 h-20 object-contain rounded-lg border border-border-dark bg-slate-950 p-1">
    ...
    <form id="delete-image-form" action="{{ route('admin.categories.image.delete', $category->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus gambar kategori ini?');" style="display:none;">
        @csrf
        @method('DELETE')
    </form>
    @endif
